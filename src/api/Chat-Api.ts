...
export async function findAllChatOfStaff(chatRoomId: number): Promise<ChatResponse[]> {
    let url: string = `http://localhost:8083/all-api/findAllChatOfStaff?userId=${getUserToken().userId}&chatRoomId=${chatRoomId}`;
